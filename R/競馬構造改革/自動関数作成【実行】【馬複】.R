...
url <- paste("R出力\\",format(Sys.Date(),"%Y%m%d"),"_","Umahuku",BFKekkaNeraiStart ,"_",BFKekkaNeraiEnd ,"_","自動生成.txt",sep="")
...
for(i in 1:length(joken)){
  ...
  selected<-subset(UmahukuMat ,UmahukuMat["馬複人気順18番"] < 80.00 &
...
　　#指数
  checkedShisu<- kekkaCheckShisu2(selected,BFKekkaNeraiStart,BFKekkaNeraiEnd ,blnShisu   )
  }
  if(length(checkedShisu[[2]][,1])>0){
  riekiritsuShisu<- sum(checkedShisu [[2]]["馬複金額"])/(length(selected[,1]) *(BFKekkaNeraiEnd-BFKekkaNeraiStart-1) * 100 - sum(checkedShisu[[1]][4])* 100)
  }
...
print(joken[i])
print(riekiritsuShisu)
　　#通常
  riekiritsu<-0
  checked<-matrix(1,0,1)
  if(length(selected[,1])>0){
  checked <- kekkaCheck2(selected,BFKekkaNeraiStart,BFKekkaNeraiEnd   )
  }
  if(length(checked [,1])>0){
  riekiritsu<-sum(checked ["馬複金額"])/(length(selected[,1]) * (BFKekkaNeraiEnd -BFKekkaNeraiStart -1)*100)
  }
