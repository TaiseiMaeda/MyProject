...
url <- paste("R出力\\",format(Sys.Date(),"%Y%m%d"),"_","UmahukuIkajo",BFKekkaNeraiStart ,"_",BFKekkaNeraiEnd ,"_",fitted,".txt",sep="")
sink(file = url ,split = TRUE)
url 
#____________________________________________________________________
#●関数
#____________________________________________________________________
keikoChosa_Ikajo<- function(KEIKODATA,STRNERAI,KEKKASTART,KEKKAEND){
...
for (j in jIdx){
  for (k in kIdx){
    ...
    if(!is.na(sub2[j,84 + k*3 + 1]) && !is.na(sub2[j,84 + k*3 + 2])){
      if(sub2[j,84 + k*3 + 1] == sub2[j,63] && sub2[j,84 + k*3 + 2] == sub2[j,64]){
...
for (i in 1:15){
  x1<-keikoChosa_Ikajo(UmahukuMat,i+3,BFKekkaNeraiStart,BFKekkaNeraiEnd)
